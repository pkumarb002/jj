...
def start_command(message):
    welcome_message = """
    🌟 Welcome to the **Maikechdoo DDoS Bot**! 🌟
...
def handle_bgmi(message):
    ...
    if not port.isdigit() or not (1 <= int(port) <= 65535):
        bot.reply_to(message, "❌ Invalid port! Please provide a port number between 10000 and 30000.")
        return
    ...
    if len(current_active) >= 1:
        bot.reply_to(message, "🚨 Maximum of 1 concurrent attacks allowed. Please wait for the current attack to finish before launching a new one.")
        return
